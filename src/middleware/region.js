...
const regions = (code = '') => {
  ...
  if (code === '') {
    let regionsData = []
    for (const countryCode in regionList) {
      if (Object.hasOwnProperty.call(regionList, countryCode)) {
        const country = regionList[countryCode]
        regionsData.push({
          code: countryCode,
          name: country.name,
        })
    ...
    const country = regionList[code]
    return {
      code: code,
      name: country?.name,
      language: intersection(defaultLanguage, country?.languages),
      currency: country?.currency,
      timezone: ct.getCountry(code).timezones,
    }
  }
}
